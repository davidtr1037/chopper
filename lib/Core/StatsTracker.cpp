...
public:
  ...
  void run(ExecutionState *es) { statsTracker->writeIStats(); }
};
...
public:
  ...
  void run(ExecutionState *es) { statsTracker->writeStatsLine(); }
};
...
public:
  ...
  void run(ExecutionState *es) { statsTracker->computeDistances(es); }
};
}
...
void StatsTracker::computeDistances(ExecutionState *es) {
  ...
  computeDistToUncovered();
  computeDistToCall(es);
}

void StatsTracker::computeDistToReturn() {
  KModule *km = executor.kmodule;
  Module *m = km->module;
  ...
  do {
    ...
    for (std::vector<Instruction *>::iterator it = instructions.begin(),
      ...
      if (bestThrough) {
        ...
        if (best != cur ||
            (inst == f->begin()->begin() && functionShortestPath[f] != best)) {
...
void StatsTracker::computeDistToUncovered() {
  KModule *km = executor.kmodule;
  Module *m = km->module;
  const InstructionInfoTable &infos = *km->infos;
  StatisticManager &sm = *theStatisticManager;
...
void StatsTracker::computeDistToCall(ExecutionState *es) {
  if (!es || es->path.empty()) {
    return;
  }
  ...
  if (target.first && target.second) {
    ...
    do {
      ...
      for (std::vector<Instruction *>::iterator it = instructions.begin(),
        ...
        if (best != cur) {
          sm.setIndexedValue(stats::minDistToCall, infos.getInfo(inst).id,
                             best);
          changed = true;
        }
      }
